...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if Recipe.objects.filter(user=('user'), recipe__id=('pk'),).exists():
            raise serializers.ValidationError(
                'Ошибка добавления рецепта в список')
        
        if not Recipe.objects.filter(user=('user'), author=('author'),).exists():
            raise serializers.ValidationError(
                'Ошибка удаления рецепта из списка')
        return data
...
class FollowSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data('user',) == data('author',):
            raise serializers.ValidationError(
                'Ошибка подписки, нельзя подписываться на себя')
